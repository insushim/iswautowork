// 과목별/학년별 공통 문장 (성취기준과 무관한 일반적 평가 문장)
// 각 성취수준별로 학년에 맞는 표현 사용

import { SubjectAchievementLevel } from '@/types';

type GradeGroup = '1-2' | '3-4' | '5-6';

interface CommonPhrases {
  excellent: string[];      // 상
  good: string[];           // 중상
  average: string[];        // 중
  fair: string[];           // 중하
  needs_effort: string[];   // 하
}

// 학년군 판별
export function getGradeGroup(grade: number): GradeGroup {
  if (grade <= 2) return '1-2';
  if (grade <= 4) return '3-4';
  return '5-6';
}

// 과목별 공통 문장 데이터 (1-2학년 전용 과목은 1-2만 있음)
const COMMON_PHRASES: Record<string, Partial<Record<GradeGroup, CommonPhrases>>> = {
  '국어': {
    '1-2': {
      excellent: [
        '글을 읽고 내용을 정확하게 이해하는 능력이 뛰어남.',
        '자신의 생각을 또박또박 말로 잘 표현함.',
        '받아쓰기와 글씨 쓰기에서 정확성이 돋보임.',
      ],
      good: [
        '책 읽기에 흥미를 보이며 꾸준히 읽기 활동에 참여함.',
        '말하기와 듣기 활동에 적극적으로 참여함.',
        '바른 글씨체로 문장을 성실하게 씀.',
      ],
      average: [
        '글을 읽고 대략적인 내용을 이해함.',
        '발표 활동에 참여하며 자신의 생각을 표현함.',
        '문장 쓰기를 꾸준히 연습함.',
      ],
      fair: [
        '글자와 낱말 읽기에 노력하는 모습을 보임.',
        '친구들 앞에서 말하기를 시도하며 발전함.',
        '글씨 쓰기 연습을 통해 향상되고 있음.',
      ],
      needs_effort: [
        '글자 읽기에 관심을 보이며 배우고 있음.',
        '선생님 도움을 받아 말하기 활동에 참여함.',
        '글씨 쓰기를 조금씩 익혀가고 있음.',
      ],
    },
    '3-4': {
      excellent: [
        '다양한 종류의 글을 읽고 내용을 정확하게 파악하는 능력이 탁월함.',
        '자신의 생각을 조리 있게 말하고 글로 표현하는 능력이 뛰어남.',
        '어휘력이 풍부하고 맞춤법에 맞게 글을 정확하게 씀.',
      ],
      good: [
        '책 읽기를 즐기며 다양한 글감에 관심을 보임.',
        '토의와 발표 활동에 적극적으로 참여하며 의견을 잘 전달함.',
        '글쓰기에 성실하게 임하며 꾸준히 발전함.',
      ],
      average: [
        '글을 읽고 중심 내용을 이해함.',
        '발표 활동에 참여하며 자신의 의견을 표현함.',
        '문장을 만들어 글로 표현할 수 있음.',
      ],
      fair: [
        '글 읽기에 노력하며 내용 이해력이 향상되고 있음.',
        '발표 활동에 참여하려는 의지를 보임.',
        '글쓰기 연습을 통해 문장력이 발전함.',
      ],
      needs_effort: [
        '글 읽기에 흥미를 갖기 시작함.',
        '친구들의 발표를 듣고 따라서 말해보려고 시도함.',
        '짧은 문장 쓰기를 연습하며 익히고 있음.',
      ],
    },
    '5-6': {
      excellent: [
        '다양한 글을 비판적으로 읽고 핵심을 정확하게 파악하는 능력이 탁월함.',
        '자신의 생각을 논리적으로 구성하여 설득력 있게 표현함.',
        '맞춤법과 문법에 맞는 정확한 글쓰기 능력이 뛰어남.',
      ],
      good: [
        '다양한 장르의 글을 즐겨 읽으며 독서 습관이 잘 형성되어 있음.',
        '토론과 발표에서 적극적으로 자신의 의견을 개진함.',
        '글의 구조를 고려하여 체계적으로 글을 씀.',
      ],
      average: [
        '글을 읽고 내용을 이해하며 요약할 수 있음.',
        '발표 활동에 참여하여 자신의 의견을 표현함.',
        '주제에 맞게 글을 작성할 수 있음.',
      ],
      fair: [
        '글 읽기에 노력하며 이해력이 향상되고 있음.',
        '발표에 참여하려는 모습을 보이며 발전함.',
        '글쓰기 활동에 꾸준히 노력함.',
      ],
      needs_effort: [
        '글 읽기에 관심을 갖고 시도함.',
        '친구들의 발표를 경청하며 배우려는 자세를 보임.',
        '글쓰기를 조금씩 연습하며 익히고 있음.',
      ],
    },
  },
  '수학': {
    '1-2': {
      excellent: [
        '수 세기와 덧셈, 뺄셈 계산이 빠르고 정확함.',
        '수학 문제를 이해하고 해결하는 능력이 뛰어남.',
        '수학적 사고력이 뛰어나 문제 상황을 잘 파악함.',
      ],
      good: [
        '수 개념을 잘 이해하고 계산에 흥미를 보임.',
        '수학 문제 풀이에 적극적으로 참여함.',
        '규칙 찾기와 수 비교를 잘함.',
      ],
      average: [
        '기본적인 수 개념을 이해하고 계산할 수 있음.',
        '수학 문제를 풀려고 노력함.',
        '수학 활동에 참여함.',
      ],
      fair: [
        '수 세기와 계산 연습에 노력함.',
        '수학 문제 이해를 위해 노력하며 발전함.',
        '수학 활동에 참여하려는 모습을 보임.',
      ],
      needs_effort: [
        '수에 관심을 갖고 세어보려고 시도함.',
        '도움을 받아 수학 활동에 참여함.',
        '수 개념을 익히기 위해 노력함.',
      ],
    },
    '3-4': {
      excellent: [
        '사칙연산 계산이 빠르고 정확하며 암산 능력이 뛰어남.',
        '수학 문제를 논리적으로 분석하고 해결하는 능력이 탁월함.',
        '수학적 개념을 정확하게 이해하고 다양한 문제에 적용함.',
      ],
      good: [
        '연산 능력이 좋고 계산에 자신감을 보임.',
        '수학 문제 풀이에 적극적이며 다양한 방법을 시도함.',
        '수학 개념을 잘 이해하고 활용함.',
      ],
      average: [
        '기본적인 연산을 수행할 수 있음.',
        '수학 문제를 이해하고 풀이를 시도함.',
        '수학 개념을 알고 문제에 적용함.',
      ],
      fair: [
        '연산 연습에 노력하며 계산력이 향상됨.',
        '수학 문제 이해에 노력하며 발전함.',
        '수학 개념 익히기에 꾸준히 노력함.',
      ],
      needs_effort: [
        '연산에 관심을 갖고 연습함.',
        '도움을 받아 수학 문제를 풀어봄.',
        '수학 개념을 배우려는 자세를 보임.',
      ],
    },
    '5-6': {
      excellent: [
        '복잡한 연산과 문제 해결에서 정확성과 속도가 뛰어남.',
        '수학적 사고력과 논리력이 탁월하여 심화 문제도 잘 해결함.',
        '수학 개념을 정확히 이해하고 실생활에 창의적으로 적용함.',
      ],
      good: [
        '연산 능력이 좋고 다양한 유형의 문제를 잘 해결함.',
        '수학적 추론을 통해 문제를 분석하고 해결함.',
        '수학 개념을 잘 이해하고 적용하는 능력이 우수함.',
      ],
      average: [
        '기본 연산과 문제 풀이를 수행할 수 있음.',
        '수학 문제를 이해하고 풀이 과정을 따라감.',
        '수학 개념을 알고 활용함.',
      ],
      fair: [
        '연산 연습에 노력하며 계산력이 발전함.',
        '문제 풀이에 노력하며 향상됨.',
        '수학 개념 익히기에 꾸준히 노력함.',
      ],
      needs_effort: [
        '연산 연습에 참여하며 익히고 있음.',
        '도움을 받아 문제를 풀어보려고 시도함.',
        '수학에 관심을 갖기 시작함.',
      ],
    },
  },
  '사회': {
    '1-2': {
      excellent: [
        '주변 환경과 사회에 대한 관심이 높고 이해력이 뛰어남.',
        '가족과 이웃의 역할을 잘 이해하고 설명함.',
        '우리 동네와 학교에 대해 잘 알고 있음.',
      ],
      good: [
        '주변 사회 현상에 관심을 갖고 탐구함.',
        '사회 수업에 적극적으로 참여함.',
        '지역 사회에 대한 이해가 좋음.',
      ],
      average: [
        '사회 현상에 대해 알고 있음.',
        '수업 활동에 참여함.',
        '지역 사회의 기본 개념을 이해함.',
      ],
      fair: [
        '사회 수업에 참여하려고 노력함.',
        '주변 환경에 관심을 갖기 시작함.',
        '사회 개념 이해에 노력함.',
      ],
      needs_effort: [
        '사회 현상에 관심을 보임.',
        '수업에 참여하려는 모습을 보임.',
        '도움을 받아 사회 개념을 익히고 있음.',
      ],
    },
    '3-4': {
      excellent: [
        '지역 사회와 우리 고장에 대한 이해가 깊고 탐구력이 뛰어남.',
        '사회 현상을 분석하고 원인과 결과를 잘 파악함.',
        '지도 읽기와 지역 탐구 능력이 뛰어남.',
      ],
      good: [
        '사회 현상에 관심이 많고 탐구 활동에 적극적임.',
        '지역 사회에 대한 이해가 좋음.',
        '사회 수업에 성실하게 참여함.',
      ],
      average: [
        '사회 현상에 대해 이해함.',
        '수업 활동에 참여하며 과제를 수행함.',
        '기본적인 사회 개념을 알고 있음.',
      ],
      fair: [
        '사회 개념 이해에 노력하며 향상됨.',
        '수업에 참여하려는 모습을 보임.',
        '지역 사회에 관심을 갖기 시작함.',
      ],
      needs_effort: [
        '사회 현상에 관심을 갖기 시작함.',
        '도움을 받아 사회 개념을 배움.',
        '수업에 참여하려고 시도함.',
      ],
    },
    '5-6': {
      excellent: [
        '사회 현상에 대한 비판적 사고력이 뛰어남.',
        '다양한 사회 문제를 분석하고 해결 방안을 제시함.',
        '사회 개념을 정확하게 이해하고 적용함.',
      ],
      good: [
        '사회 현상 탐구에 적극적으로 참여함.',
        '사회 문제에 관심을 갖고 다양한 관점에서 분석함.',
        '사회 개념을 잘 이해하고 적용함.',
      ],
      average: [
        '사회 현상을 이해함.',
        '사회 수업에 참여하며 과제를 수행함.',
        '사회 개념을 알고 활용함.',
      ],
      fair: [
        '사회 개념 이해에 노력하며 발전함.',
        '사회 학습에 관심을 갖고 노력함.',
        '수업 참여도가 향상되고 있음.',
      ],
      needs_effort: [
        '사회 현상에 관심을 보이기 시작함.',
        '사회 이야기에 흥미를 느끼기 시작함.',
        '도움을 받아 사회 개념을 익히고 있음.',
      ],
    },
  },
  '과학': {
    '1-2': {
      excellent: [
        '주변 자연 현상에 대한 호기심과 탐구력이 뛰어남.',
        '관찰 활동에서 세밀하고 정확하게 관찰함.',
        '자연 현상의 원리를 이해하고 설명하는 능력이 뛰어남.',
      ],
      good: [
        '자연 현상에 관심이 많고 탐구 활동에 적극적임.',
        '관찰 활동을 성실하게 수행함.',
        '과학 수업에 흥미를 갖고 참여함.',
      ],
      average: [
        '자연 현상을 관찰하고 이해함.',
        '과학 활동에 참여함.',
        '기본적인 과학 개념을 알고 있음.',
      ],
      fair: [
        '관찰 활동에 참여하며 노력함.',
        '자연 현상에 관심을 갖기 시작함.',
        '과학 개념 이해에 노력함.',
      ],
      needs_effort: [
        '자연 현상에 관심을 보임.',
        '도움을 받아 관찰 활동에 참여함.',
        '과학에 흥미를 갖기 시작함.',
      ],
    },
    '3-4': {
      excellent: [
        '과학적 탐구력과 실험 수행 능력이 뛰어남.',
        '과학 현상의 원리를 논리적으로 이해하고 설명함.',
        '실험 결과를 정확하게 분석하고 결론을 도출함.',
      ],
      good: [
        '과학 탐구 활동에 적극적이고 실험에 흥미를 보임.',
        '과학 개념을 잘 이해하고 적용함.',
        '관찰과 실험을 성실하게 수행함.',
      ],
      average: [
        '과학 현상을 이해하고 실험에 참여함.',
        '과학 개념을 알고 활용함.',
        '탐구 활동에 참여함.',
      ],
      fair: [
        '과학 개념 이해에 노력하며 발전함.',
        '실험 활동에 참여하려는 모습을 보임.',
        '탐구 활동에 관심을 갖기 시작함.',
      ],
      needs_effort: [
        '과학에 관심을 갖기 시작함.',
        '도움을 받아 실험에 참여함.',
        '과학 개념을 익히려고 노력함.',
      ],
    },
    '5-6': {
      excellent: [
        '과학적 사고력과 탐구 설계 능력이 탁월함.',
        '실험 결과를 분석하고 과학적 결론을 논리적으로 도출함.',
        '과학 원리를 정확히 이해하고 심화 학습에도 뛰어남.',
      ],
      good: [
        '과학 탐구와 실험에 적극적이고 호기심이 많음.',
        '과학 개념을 잘 이해하고 다양한 상황에 적용함.',
        '실험 보고서 작성과 결과 분석 능력이 좋음.',
      ],
      average: [
        '과학 현상을 이해하고 실험에 참여함.',
        '과학 개념을 알고 문제에 적용함.',
        '탐구 활동을 수행함.',
      ],
      fair: [
        '과학 개념 이해에 노력하며 향상됨.',
        '실험 활동에 참여하며 발전함.',
        '탐구 활동에 관심을 보임.',
      ],
      needs_effort: [
        '과학에 관심을 갖기 시작함.',
        '도움을 받아 실험에 참여함.',
        '과학 원리를 배우려는 자세를 보임.',
      ],
    },
  },
  '영어': {
    '1-2': {
      excellent: [
        '영어 발음이 정확하고 듣기 능력이 뛰어남.',
        '영어 단어와 표현을 빠르게 습득함.',
        '영어 수업에 자신감 있게 참여함.',
      ],
      good: [
        '영어 듣기와 말하기에 흥미를 보임.',
        '영어 단어를 잘 기억하고 활용함.',
        '영어 활동에 적극적으로 참여함.',
      ],
      average: [
        '기본적인 영어 표현을 이해함.',
        '영어 수업에 참여함.',
        '영어 단어를 익히고 있음.',
      ],
      fair: [
        '영어에 관심을 갖기 시작함.',
        '영어 활동에 참여하려고 노력함.',
        '영어 단어 익히기에 노력함.',
      ],
      needs_effort: [
        '영어에 흥미를 보이기 시작함.',
        '도움을 받아 영어 활동에 참여함.',
        '영어 표현을 조금씩 익히고 있음.',
      ],
    },
    '3-4': {
      excellent: [
        '영어 듣기, 말하기, 읽기 능력이 고르게 뛰어남.',
        '영어 문장을 정확하게 이해하고 표현함.',
        '영어 의사소통에 자신감을 보이며 적극적으로 참여함.',
      ],
      good: [
        '영어 학습에 적극적이고 발음이 좋음.',
        '영어 표현을 잘 활용하여 대화함.',
        '영어 읽기와 쓰기에 흥미를 보임.',
      ],
      average: [
        '기본적인 영어 표현을 이해하고 활용함.',
        '영어 수업에 참여하며 학습함.',
        '영어 단어와 문장을 익히고 있음.',
      ],
      fair: [
        '영어 학습에 노력하며 발전함.',
        '영어 표현 익히기에 꾸준히 노력함.',
        '영어 수업 참여도가 향상됨.',
      ],
      needs_effort: [
        '영어에 관심을 갖기 시작함.',
        '도움을 받아 영어 표현을 배움.',
        '영어 활동에 참여하려고 시도함.',
      ],
    },
    '5-6': {
      excellent: [
        '영어 4기능(듣기, 말하기, 읽기, 쓰기)이 고르게 우수함.',
        '영어로 자신의 생각을 자연스럽게 표현함.',
        '영어 의사소통 능력이 뛰어나고 자신감이 넘침.',
      ],
      good: [
        '영어 학습에 적극적이고 실력이 꾸준히 향상됨.',
        '영어 대화와 읽기에 자신감을 보임.',
        '영어 표현력이 좋고 어휘력이 풍부함.',
      ],
      average: [
        '기본적인 영어 의사소통이 가능함.',
        '영어 수업에 참여하며 학습함.',
        '영어 표현을 이해하고 활용함.',
      ],
      fair: [
        '영어 학습에 노력하며 향상됨.',
        '영어 표현 익히기에 꾸준히 노력함.',
        '영어 수업 참여도가 발전함.',
      ],
      needs_effort: [
        '영어에 관심을 갖기 시작함.',
        '도움을 받아 영어를 배우고 있음.',
        '기본 영어 표현을 익히려고 시도함.',
      ],
    },
  },
  '도덕': {
    '1-2': {
      excellent: [
        '바른 생활 습관이 잘 형성되어 있고 예절이 바름.',
        '친구를 배려하고 도우려는 마음이 뛰어남.',
        '규칙을 잘 지키고 책임감이 강함.',
      ],
      good: [
        '바른 행동을 실천하려고 노력함.',
        '친구와 사이좋게 지내며 배려심이 있음.',
        '약속과 규칙을 지키려고 노력함.',
      ],
      average: [
        '기본 예절을 알고 지킴.',
        '친구와 어울려 지냄.',
        '규칙을 따름.',
      ],
      fair: [
        '바른 생활을 위해 노력함.',
        '친구와 잘 지내려고 노력함.',
        '규칙 지키기를 연습함.',
      ],
      needs_effort: [
        '바른 행동에 관심을 갖기 시작함.',
        '친구 사귀기를 시도함.',
        '규칙을 배우고 있음.',
      ],
    },
    '3-4': {
      excellent: [
        '도덕적 판단력이 뛰어나고 올바른 행동을 실천함.',
        '타인을 배려하고 존중하는 태도가 몸에 배어 있음.',
        '정직하고 성실하며 책임감이 강함.',
      ],
      good: [
        '도덕적 가치를 이해하고 실천하려고 노력함.',
        '친구를 배려하고 협동하는 자세가 좋음.',
        '규칙을 잘 지키고 약속을 이행함.',
      ],
      average: [
        '도덕적 가치를 알고 따름.',
        '친구와 협력하며 지냄.',
        '규칙을 준수함.',
      ],
      fair: [
        '도덕적 행동을 위해 노력함.',
        '배려하는 마음을 기르려고 함.',
        '규칙 지키기에 노력함.',
      ],
      needs_effort: [
        '도덕적 가치에 관심을 갖기 시작함.',
        '타인과 어울리려고 시도함.',
        '규칙의 필요성을 배우고 있음.',
      ],
    },
    '5-6': {
      excellent: [
        '높은 도덕성을 갖추고 올바른 판단과 행동을 실천함.',
        '공동체 의식이 뛰어나고 봉사 정신이 투철함.',
        '정의롭고 책임감 있는 태도로 생활함.',
      ],
      good: [
        '도덕적 가치를 실천하려는 의지가 강함.',
        '타인을 존중하고 배려하는 마음이 깊음.',
        '책임감을 갖고 맡은 일을 성실히 수행함.',
      ],
      average: [
        '도덕적 가치를 이해하고 따름.',
        '친구들과 협력하며 생활함.',
        '규칙과 질서를 준수함.',
      ],
      fair: [
        '도덕적 성장을 위해 노력함.',
        '배려와 협동 자세가 향상됨.',
        '책임감을 기르려고 노력함.',
      ],
      needs_effort: [
        '도덕적 가치에 관심을 갖기 시작함.',
        '공동체 생활에 참여하려고 시도함.',
        '규칙의 중요성을 배우고 있음.',
      ],
    },
  },
  '체육': {
    '1-2': {
      excellent: [
        '다양한 신체 활동에서 운동 능력이 뛰어남.',
        '규칙을 잘 지키며 게임에 적극적으로 참여함.',
        '신체 움직임이 활발하고 체력이 좋음.',
      ],
      good: [
        '체육 활동에 적극적이고 즐겁게 참여함.',
        '친구들과 협력하여 게임을 잘함.',
        '기본 운동 기능이 좋음.',
      ],
      average: [
        '체육 활동에 참여함.',
        '기본적인 운동을 수행함.',
        '규칙을 따라 게임에 참여함.',
      ],
      fair: [
        '체육 활동에 참여하려고 노력함.',
        '운동 기능이 향상되고 있음.',
        '친구들과 함께 활동하려고 함.',
      ],
      needs_effort: [
        '신체 활동에 관심을 보임.',
        '도움을 받아 운동에 참여함.',
        '체육 활동을 시도함.',
      ],
    },
    '3-4': {
      excellent: [
        '다양한 운동 기능이 뛰어나고 체력이 우수함.',
        '스포츠맨십을 갖추고 경기에서 두각을 나타냄.',
        '팀 활동에서 리더십을 발휘함.',
      ],
      good: [
        '체육 활동에 적극적이고 운동 능력이 좋음.',
        '규칙을 잘 지키며 정정당당하게 경기함.',
        '협동심을 갖고 팀 활동에 참여함.',
      ],
      average: [
        '기본적인 운동 기능을 수행함.',
        '체육 활동에 참여함.',
        '규칙을 따라 게임에 참여함.',
      ],
      fair: [
        '운동 기능이 향상되고 있음.',
        '체육 활동 참여에 노력함.',
        '협동 활동에 참여하려고 함.',
      ],
      needs_effort: [
        '체육 활동에 관심을 보임.',
        '도움을 받아 운동에 참여함.',
        '기본 운동을 익히고 있음.',
      ],
    },
    '5-6': {
      excellent: [
        '뛰어난 운동 능력과 체력으로 다양한 스포츠에서 우수한 성과를 보임.',
        '스포츠맨십이 뛰어나고 페어플레이 정신이 투철함.',
        '팀 활동에서 리더십과 협동심을 발휘함.',
      ],
      good: [
        '체육 활동에 적극적이고 운동 기능이 우수함.',
        '건강 관리와 체력 향상에 관심이 많음.',
        '협동 정신을 갖고 팀 활동에 잘 참여함.',
      ],
      average: [
        '기본적인 운동 기능을 수행함.',
        '체육 활동에 참여함.',
        '규칙을 지키며 경기에 참여함.',
      ],
      fair: [
        '운동 기능이 향상되고 있음.',
        '체육 활동 참여에 노력함.',
        '체력 증진을 위해 노력함.',
      ],
      needs_effort: [
        '체육 활동에 관심을 보임.',
        '도움을 받아 운동에 참여함.',
        '기본 운동을 연습함.',
      ],
    },
  },
  '음악': {
    '1-2': {
      excellent: [
        '리듬감과 음감이 뛰어나고 노래를 잘 부름.',
        '음악 활동에 즐겁게 참여하며 표현력이 좋음.',
        '악기 연주에 관심이 많고 소질을 보임.',
      ],
      good: [
        '음악 수업에 즐겁게 참여함.',
        '노래 부르기와 리듬 활동을 잘함.',
        '음악에 대한 흥미가 높음.',
      ],
      average: [
        '음악 활동에 참여함.',
        '노래를 따라 부름.',
        '리듬에 맞춰 활동함.',
      ],
      fair: [
        '음악 활동에 참여하려고 노력함.',
        '노래 부르기를 시도함.',
        '음악에 관심을 갖기 시작함.',
      ],
      needs_effort: [
        '음악에 흥미를 보이기 시작함.',
        '도움을 받아 음악 활동에 참여함.',
        '음악 감상에 관심을 보임.',
      ],
    },
    '3-4': {
      excellent: [
        '음악적 재능이 뛰어나고 다양한 음악 활동에서 능력을 발휘함.',
        '악기 연주 실력이 좋고 음악적 표현력이 풍부함.',
        '음악 감상에서 깊이 있는 이해를 보임.',
      ],
      good: [
        '음악 수업에 적극적이고 음악적 표현력이 좋음.',
        '노래와 악기 연주에 흥미를 보임.',
        '음악 활동에 성실하게 참여함.',
      ],
      average: [
        '음악 활동에 참여함.',
        '노래를 부르고 악기를 다룸.',
        '음악 감상에 참여함.',
      ],
      fair: [
        '음악 활동 참여에 노력함.',
        '음악적 표현이 향상되고 있음.',
        '악기 다루기를 연습함.',
      ],
      needs_effort: [
        '음악에 관심을 갖기 시작함.',
        '도움을 받아 음악 활동에 참여함.',
        '음악 감상을 시도함.',
      ],
    },
    '5-6': {
      excellent: [
        '뛰어난 음악적 재능으로 연주와 가창에서 우수한 실력을 보임.',
        '음악적 감수성이 풍부하고 표현력이 탁월함.',
        '다양한 음악 장르를 이해하고 감상하는 능력이 뛰어남.',
      ],
      good: [
        '음악 활동에 적극적이고 음악적 소양이 좋음.',
        '악기 연주와 가창에 자신감을 보임.',
        '음악 감상에서 깊이 있는 이해를 보임.',
      ],
      average: [
        '음악 활동에 참여함.',
        '노래와 악기 연주를 수행함.',
        '음악 감상에 참여함.',
      ],
      fair: [
        '음악 활동 참여에 노력함.',
        '음악적 표현이 향상됨.',
        '악기 다루기를 연습함.',
      ],
      needs_effort: [
        '음악에 관심을 갖기 시작함.',
        '도움을 받아 음악 활동에 참여함.',
        '음악 감상을 시도함.',
      ],
    },
  },
  '미술': {
    '1-2': {
      excellent: [
        '창의적인 표현력이 뛰어나고 미술 활동에 재능을 보임.',
        '다양한 재료를 활용하여 작품을 잘 만듦.',
        '색채 감각과 조형 능력이 우수함.',
      ],
      good: [
        '미술 활동에 즐겁게 참여하고 표현력이 좋음.',
        '그리기와 만들기에 흥미를 보임.',
        '작품 제작에 정성을 기울임.',
      ],
      average: [
        '미술 활동에 참여함.',
        '주제에 맞게 그림을 그림.',
        '다양한 재료를 사용해봄.',
      ],
      fair: [
        '미술 활동에 참여하려고 노력함.',
        '그리기와 만들기를 시도함.',
        '미술에 관심을 갖기 시작함.',
      ],
      needs_effort: [
        '미술에 흥미를 보이기 시작함.',
        '도움을 받아 미술 활동에 참여함.',
        '작품 만들기를 시도함.',
      ],
    },
    '3-4': {
      excellent: [
        '창의적인 발상과 표현력이 뛰어나 작품 활동에서 두각을 나타냄.',
        '다양한 미술 기법을 익히고 활용하는 능력이 우수함.',
        '작품 감상에서 깊이 있는 이해와 비평 능력을 보임.',
      ],
      good: [
        '미술 활동에 적극적이고 표현력이 좋음.',
        '다양한 재료와 기법을 탐색하며 작품을 제작함.',
        '미술 작품 감상에 흥미를 보임.',
      ],
      average: [
        '미술 활동에 참여함.',
        '주제에 맞게 작품을 제작함.',
        '작품 감상에 참여함.',
      ],
      fair: [
        '미술 활동 참여에 노력함.',
        '표현 능력이 향상되고 있음.',
        '작품 제작을 시도함.',
      ],
      needs_effort: [
        '미술에 관심을 갖기 시작함.',
        '도움을 받아 미술 활동에 참여함.',
        '작품 만들기를 시도함.',
      ],
    },
    '5-6': {
      excellent: [
        '뛰어난 미술적 재능으로 창의적이고 완성도 높은 작품을 제작함.',
        '다양한 미술 기법과 재료를 능숙하게 다루며 표현력이 탁월함.',
        '미술 작품에 대한 깊이 있는 감상과 비평 능력을 갖춤.',
      ],
      good: [
        '미술 활동에 적극적이고 창의적인 표현력이 좋음.',
        '다양한 기법을 시도하며 작품의 완성도를 높임.',
        '작품 감상에서 의미 있는 해석을 함.',
      ],
      average: [
        '미술 활동에 참여함.',
        '주제에 맞게 작품을 제작함.',
        '작품 감상에 참여함.',
      ],
      fair: [
        '미술 활동 참여에 노력함.',
        '표현 능력이 향상됨.',
        '작품 제작에 노력함.',
      ],
      needs_effort: [
        '미술에 관심을 갖기 시작함.',
        '도움을 받아 미술 활동에 참여함.',
        '작품 만들기를 시도함.',
      ],
    },
  },
  '실과': {
    '1-2': {
      excellent: [
        '생활 기술에 대한 이해가 뛰어나고 실습을 잘함.',
        '손재주가 좋고 만들기 활동에서 우수한 결과를 보임.',
        '안전 수칙을 잘 지키며 활동함.',
      ],
      good: [
        '실과 활동에 적극적으로 참여함.',
        '실습에 흥미를 갖고 성실하게 수행함.',
        '생활 기술을 잘 익힘.',
      ],
      average: [
        '실과 활동에 참여함.',
        '기본적인 실습을 수행함.',
        '생활 기술을 배움.',
      ],
      fair: [
        '실과 활동 참여에 노력함.',
        '실습 능력이 향상됨.',
        '생활 기술 익히기에 노력함.',
      ],
      needs_effort: [
        '실과에 관심을 갖기 시작함.',
        '도움을 받아 실습에 참여함.',
        '만들기를 시도함.',
      ],
    },
    '3-4': {
      excellent: [
        '실과 기술에 대한 이해력과 응용력이 뛰어남.',
        '실습 활동에서 창의적이고 완성도 높은 결과물을 만듦.',
        '기술 활용 능력이 우수하고 안전 의식이 높음.',
      ],
      good: [
        '실과 활동에 적극적이고 실습 능력이 좋음.',
        '다양한 기술을 익히고 활용함.',
        '성실하게 실습에 참여함.',
      ],
      average: [
        '실과 활동에 참여함.',
        '기본적인 기술을 익히고 활용함.',
        '실습 과제를 수행함.',
      ],
      fair: [
        '실과 활동 참여에 노력함.',
        '기술 익히기에 노력함.',
        '실습 능력이 향상됨.',
      ],
      needs_effort: [
        '실과에 관심을 갖기 시작함.',
        '도움을 받아 실습에 참여함.',
        '기술 익히기를 시도함.',
      ],
    },
    '5-6': {
      excellent: [
        '다양한 실과 기술을 능숙하게 익히고 창의적으로 활용함.',
        '소프트웨어와 기술 활용 능력이 뛰어남.',
        '실생활 문제를 기술로 해결하는 능력이 우수함.',
      ],
      good: [
        '실과 활동에 적극적이고 기술 습득이 빠름.',
        '다양한 기술을 익히고 적용하는 능력이 좋음.',
        '안전하게 도구를 다루며 실습함.',
      ],
      average: [
        '실과 활동에 참여함.',
        '기본적인 기술을 수행함.',
        '실습 과제를 완수함.',
      ],
      fair: [
        '실과 활동 참여에 노력함.',
        '기술 익히기에 노력함.',
        '실습 능력이 향상됨.',
      ],
      needs_effort: [
        '실과에 관심을 갖기 시작함.',
        '도움을 받아 실습에 참여함.',
        '기술 익히기를 시도함.',
      ],
    },
  },
  '바른생활': {
    '1-2': {
      excellent: [
        '바른 생활 습관이 잘 형성되어 있고 모범이 됨.',
        '예절 바르고 친구를 잘 배려함.',
        '규칙을 스스로 잘 지킴.',
      ],
      good: [
        '바른 행동을 실천하려고 노력함.',
        '친구와 사이좋게 지냄.',
        '약속을 잘 지킴.',
      ],
      average: [
        '기본 예절을 지킴.',
        '친구와 어울림.',
        '규칙을 따름.',
      ],
      fair: [
        '바른 생활 습관을 기르려고 노력함.',
        '친구와 잘 지내려고 함.',
        '규칙 지키기를 연습함.',
      ],
      needs_effort: [
        '바른 행동에 관심을 갖기 시작함.',
        '친구 사귀기를 시도함.',
        '규칙을 배우고 있음.',
      ],
    },
  },
  '슬기로운생활': {
    '1-2': {
      excellent: [
        '주변 세계에 대한 호기심과 탐구력이 뛰어남.',
        '관찰력이 좋고 특징을 잘 파악함.',
        '다양한 현상의 규칙을 발견하는 능력이 우수함.',
      ],
      good: [
        '탐구 활동에 적극적으로 참여함.',
        '관찰 활동을 성실하게 수행함.',
        '주변 현상에 관심이 많음.',
      ],
      average: [
        '탐구 활동에 참여함.',
        '관찰을 하고 결과를 이야기함.',
        '주변 환경에 관심을 가짐.',
      ],
      fair: [
        '탐구 활동에 참여하려고 노력함.',
        '관찰하기를 시도함.',
        '주변에 관심을 갖기 시작함.',
      ],
      needs_effort: [
        '탐구에 흥미를 보이기 시작함.',
        '도움을 받아 관찰 활동에 참여함.',
        '주변 현상에 관심을 보임.',
      ],
    },
  },
  '즐거운생활': {
    '1-2': {
      excellent: [
        '표현 활동에서 창의적이고 풍부한 표현력을 보임.',
        '음악과 미술, 신체 표현을 모두 잘함.',
        '다양한 놀이와 활동에 즐겁게 참여함.',
      ],
      good: [
        '표현 활동에 적극적으로 참여함.',
        '음악, 미술, 신체 활동에 흥미를 보임.',
        '놀이에 즐겁게 참여함.',
      ],
      average: [
        '표현 활동에 참여함.',
        '노래와 그리기를 함.',
        '놀이에 참여함.',
      ],
      fair: [
        '표현 활동에 참여하려고 노력함.',
        '활동에 관심을 갖기 시작함.',
        '친구들과 놀이하려고 함.',
      ],
      needs_effort: [
        '표현에 흥미를 보이기 시작함.',
        '도움을 받아 활동에 참여함.',
        '놀이에 관심을 보임.',
      ],
    },
  },
};

// 학기별 세분화된 공통 문장 (5-6학년 사회처럼 학기별 내용이 다른 경우)
// key: `${subjectName}_${grade}_${semester}`
const SEMESTER_SPECIFIC_PHRASES: Record<string, CommonPhrases> = {
  // ========== 사회 5학년 1학기: 국토와 인권 ==========
  '사회_5_1': {
    excellent: [
      '우리나라 국토에 대한 이해가 깊고 지리 정보 활용 능력이 뛰어남.',
      '인권의 의미를 정확히 이해하고 일상에서 실천하는 태도가 돋보임.',
      '사회 현상을 비판적으로 분석하고 자신의 의견을 논리적으로 표현함.',
    ],
    good: [
      '국토와 지리에 대한 관심이 높고 탐구 활동에 적극 참여함.',
      '인권과 법에 대한 이해가 좋고 사회 정의에 관심을 보임.',
      '사회 문제에 관심을 갖고 다양한 관점에서 분석함.',
    ],
    average: [
      '우리나라 국토의 특성을 이해함.',
      '인권의 기본 개념을 알고 수업에 참여함.',
      '사회 개념을 알고 활용함.',
    ],
    fair: [
      '지리 개념 이해에 노력하며 발전함.',
      '인권과 법에 대한 학습에 노력함.',
      '수업 참여도가 향상되고 있음.',
    ],
    needs_effort: [
      '국토와 지리에 관심을 갖기 시작함.',
      '사회 개념을 익히려고 노력함.',
      '도움을 받아 수업에 참여함.',
    ],
  },
  // ========== 사회 5학년 2학기: 역사 (고조선~조선, 근현대사) ==========
  '사회_5_2': {
    excellent: [
      '역사에 대한 관심이 높고 역사적 사고력이 뛰어남.',
      '역사적 사건의 원인과 결과를 정확히 분석하는 능력이 탁월함.',
      '역사 자료를 비판적으로 분석하고 해석하는 능력이 우수함.',
    ],
    good: [
      '역사 탐구에 적극적으로 참여하고 관련 지식이 풍부함.',
      '역사적 사건에 관심을 갖고 다양한 자료를 탐색함.',
      '역사 수업에 성실하게 참여하고 과제를 잘 수행함.',
    ],
    average: [
      '역사적 사건과 인물에 대해 이해함.',
      '역사 수업에 참여하며 과제를 수행함.',
      '역사 개념을 알고 활용함.',
    ],
    fair: [
      '역사 학습에 노력하며 이해력이 향상됨.',
      '역사적 사건에 관심을 갖기 시작함.',
      '수업 참여도가 향상되고 있음.',
    ],
    needs_effort: [
      '역사 이야기에 흥미를 느끼기 시작함.',
      '역사 개념을 익히려고 노력함.',
      '도움을 받아 역사 학습에 참여함.',
    ],
  },
  // ========== 사회 6학년 1학기: 민주주의와 경제 ==========
  '사회_6_1': {
    excellent: [
      '민주주의와 경제에 대한 이해가 깊고 사회적 사고력이 뛰어남.',
      '정치와 경제 현상을 비판적으로 분석하는 능력이 탁월함.',
      '사회 문제에 대한 해결 방안을 창의적으로 제시함.',
    ],
    good: [
      '민주주의 원리와 경제 개념을 잘 이해하고 적용함.',
      '정치와 경제에 관심을 갖고 다양한 관점에서 분석함.',
      '사회 수업에 적극적으로 참여함.',
    ],
    average: [
      '민주주의와 경제의 기본 개념을 이해함.',
      '사회 수업에 참여하며 과제를 수행함.',
      '사회 개념을 알고 활용함.',
    ],
    fair: [
      '민주주의와 경제 개념 이해에 노력함.',
      '사회 학습에 관심을 갖고 노력함.',
      '수업 참여도가 향상되고 있음.',
    ],
    needs_effort: [
      '민주주의와 경제에 관심을 갖기 시작함.',
      '사회 개념을 익히려고 노력함.',
      '도움을 받아 수업에 참여함.',
    ],
  },
  // ========== 사회 6학년 2학기: 세계 지리와 지구촌 ==========
  '사회_6_2': {
    excellent: [
      '세계 여러 나라와 지구촌 문제에 대한 이해가 깊고 탐구력이 뛰어남.',
      '세계 지리와 국제 문제를 비판적으로 분석하는 능력이 탁월함.',
      '지구촌 문제 해결에 대한 관심이 높고 세계 시민 의식이 돋보임.',
    ],
    good: [
      '세계 여러 나라의 특성을 잘 이해하고 비교 분석함.',
      '지구촌 문제에 관심을 갖고 다양한 관점에서 탐구함.',
      '사회 수업에 적극적으로 참여함.',
    ],
    average: [
      '세계 지리와 지구촌 문제의 기본 개념을 이해함.',
      '사회 수업에 참여하며 과제를 수행함.',
      '사회 개념을 알고 활용함.',
    ],
    fair: [
      '세계 지리 개념 이해에 노력하며 발전함.',
      '지구촌 문제에 관심을 갖기 시작함.',
      '수업 참여도가 향상되고 있음.',
    ],
    needs_effort: [
      '세계 여러 나라에 관심을 갖기 시작함.',
      '사회 개념을 익히려고 노력함.',
      '도움을 받아 수업에 참여함.',
    ],
  },
};

// 성취수준 매핑
function getLevelKey(level: SubjectAchievementLevel): keyof CommonPhrases {
  const mapping: Record<SubjectAchievementLevel, keyof CommonPhrases> = {
    excellent: 'excellent',
    good: 'good',
    average: 'average',
    fair: 'fair',
    needs_effort: 'needs_effort',
  };
  return mapping[level];
}

// 공통 문장 랜덤 선택
export function getRandomCommonPhrase(
  subjectName: string,
  grade: number,
  level: SubjectAchievementLevel
): string {
  const gradeGroup = getGradeGroup(grade);
  const levelKey = getLevelKey(level);

  const subjectPhrases = COMMON_PHRASES[subjectName];
  if (!subjectPhrases) {
    // 해당 과목이 없으면 기본 문구 반환
    return '';
  }

  const gradePhrases = subjectPhrases[gradeGroup];
  if (!gradePhrases) {
    return '';
  }

  const phrases = gradePhrases[levelKey];
  if (!phrases || phrases.length === 0) {
    return '';
  }

  // 랜덤 선택
  const randomIndex = Math.floor(Math.random() * phrases.length);
  return phrases[randomIndex];
}

// 모든 공통 문장 가져오기 (프롬프트에 포함용)
// semester 파라미터 추가 - 학기별로 다른 내용을 가르치는 과목(사회 5-6학년)에서 사용
export function getAllCommonPhrases(
  subjectName: string,
  grade: number,
  semester?: 1 | 2
): Record<string, string[]> {
  // 1. 학기별 세분화된 공통 문장이 있는지 먼저 확인 (5-6학년 사회 등)
  if (semester && grade >= 5) {
    const semesterKey = `${subjectName}_${grade}_${semester}`;
    const semesterPhrases = SEMESTER_SPECIFIC_PHRASES[semesterKey];
    if (semesterPhrases) {
      return {
        '상': semesterPhrases.excellent,
        '중상': semesterPhrases.good,
        '중': semesterPhrases.average,
        '중하': semesterPhrases.fair,
        '하': semesterPhrases.needs_effort,
      };
    }
  }

  // 2. 학기별 세분화가 없으면 기존 학년군별 공통 문장 사용
  const gradeGroup = getGradeGroup(grade);

  const subjectPhrases = COMMON_PHRASES[subjectName];
  if (!subjectPhrases) {
    return {};
  }

  const gradePhrases = subjectPhrases[gradeGroup];
  if (!gradePhrases) {
    return {};
  }

  return {
    '상': gradePhrases.excellent,
    '중상': gradePhrases.good,
    '중': gradePhrases.average,
    '중하': gradePhrases.fair,
    '하': gradePhrases.needs_effort,
  };
}
